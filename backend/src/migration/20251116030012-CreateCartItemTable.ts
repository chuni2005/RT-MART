...
export class CreateCartItemTable20251116030012 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // -- 複合索引 (唯一約束)
    await queryRunner.query(
      `CREATE UNIQUE INDEX UQ_cartitem_cart_id_product_id ON CartItem(cart_id, product_id)`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP INDEX IDX_cartitem_cart_id ON CartItem`);
    await queryRunner.query(
      `DROP INDEX UQ_cartitem_cart_id_product_id ON CartItem`,
    );

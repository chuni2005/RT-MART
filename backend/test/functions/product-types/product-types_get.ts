...
export async function getProductTypesWithFilterWithAdminRole(
...
export async function getProductTypesWithNonPermissionRole(
  app: INestApplication,
) {
  const res = await request(app.getHttpServer())
    .get(`/product-types/admin`)
...
export async function getSingleProductTypeByNonExistentId(
  app: INestApplication,
) {
  await request(app.getHttpServer())
...
export async function getSingleProductTypeWithNonActiveId(
  app: INestApplication,
) {
  await request(app.getHttpServer())
...
export async function getSingleProductTypeByIdWithAdminRole(
  app: INestApplication,
) {
  await request(app.getHttpServer())
...
export async function getSingleProductTypeByNonExistentIdWithAdminRole(
  app: INestApplication,
) {
  await request(app.getHttpServer())
...
export async function getSingleProductTypeWithNonPermissionRole(
  app: INestApplication,
) {
  await request(app.getHttpServer())
    .get(`/product-types/admin/${productType.productTypeId}`)

...
export async function updateProductTypeByIdWithAdminRole(
...
export async function updateProductTypeWithNonExistentIdWithAdminRole(
  app: INestApplication,
) {
  await request(app.getHttpServer())
...
export async function updateProductTypeWithNonPermissionRole(
  app: INestApplication,
) {
  await request(app.getHttpServer())
    .patch(`/product-types/${productType.productTypeId}`)

...
export async function deleteProductTypeByIdWithAdminRole(
...
export async function deleteProductTypeWithNonExistentIdWithAdminRole(
  app: INestApplication,
) {
  await request(app.getHttpServer())
...
export async function deleteProductTypeWithNonPermissionRole(
  app: INestApplication,
) {
  await request(app.getHttpServer())
    .delete(`/product-types/${productType.productTypeId}`)

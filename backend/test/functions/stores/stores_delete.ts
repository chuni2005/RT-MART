...
export async function deleteStoreByIdWithNonPermissionRole(app: INestApplication){
    await request(app.getHttpServer())
        .delete(`/stores/${buyerUser.storeId}`)
...
export async function permanentlyDeleteStoreById(app: INestApplication, storeId: Number){
...
export async function permanentlyDeleteStoreByNonExistentId(app: INestApplication){
    await request(app.getHttpServer())
...
export async function permanentlyDeleteStoreByIdWithNonCookie(app: INestApplication){
    await request(app.getHttpServer())
        .delete(`/stores/${buyerUser.storeId}/permanent`)
        .expect(401);
}

export async function permanentlyDeleteStoreByIdWithNonPermissionRole(app: INestApplication){
    await request(app.getHttpServer())
        .delete(`/stores/${buyerUser.storeId}/permanent`)
        .set('Cookie', `accessToken=${sellerUser.cookie.accessToken}`)
        .expect(403);

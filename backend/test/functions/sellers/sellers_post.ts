...
export async function appplyOwnBuyerAccountToSellerRole(
...
export async function appplyOwnBuyerAccountToSellerRoleWithNonCookie(
  app: INestApplication,
) {
  await request(app.getHttpServer()).post(`/sellers`).expect(401);
}

export async function verifyApplicationOfSeller(
  app: INestApplication,
  buyer: any,
) {
  const res = await request(app.getHttpServer())

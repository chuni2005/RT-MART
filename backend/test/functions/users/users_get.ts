...
export async function getUsersWithFilter(app: INestApplication, role: string, search: string) {
    const res = await request(app.getHttpServer())
...
export async function getDeletedUsersWithFilterAndPage(app: INestApplication, role: string, search: string) {
    const res = await request(app.getHttpServer())
...
export async function getAllOfDeletedUsersWithNonPermissionRole(app: INestApplication) {
    const res = await request(app.getHttpServer())
        .get(`/users/deleted?page=1&limit=10`)
